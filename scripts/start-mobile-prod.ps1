...
# =====================================
$djangoCmd = "cd `"$Backend`"; `$env:DJANGO_SETTINGS_MODULE='camaleao.settings.prod';python manage.py runserver 0.0.0.0:8000"
...
if (-not $emulatorReady) {
  Write-Host "Timeout: o emulador nao iniciou dentro do tempo limite."
  exit 1
}
...
$expoRunArgs = @("cross-env",
  "APP_ENV=prod",
  "NODE_ENV=production",
  "EXPO_ENVFILE=.env.prod",
  "EXPO_DEBUG=false",
  "expo",
  "run",
  "android",
