...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        attrs["email"] = (
            attrs.get("email", "").strip().lower()
        )  # Usa o campo 'email' como 'username'
...
class UserSerializer(serializers.ModelSerializer):
        ...
        if data.get("role") == "dpo":
            ...
            if not data.get("appointment_date"):
                raise serializers.ValidationError(
                    {"appointment_date": "Este campo é obrigatório para um DPO."}
                )
            if not data.get("appointment_validity"):
                raise serializers.ValidationError(
                    {"appointment_validity": "Este campo é obrigatório para um DPO."}
                )
            if not data.get("phone_number"):
                raise serializers.ValidationError(
                    {"phone_number": "Este campo é obrigatório para um DPO."}
                )
            ...
            if existing_dpo.exists():
                raise serializers.ValidationError(
                    {"role": "Já existe um usuário com a função de DPO."}
                )
        ...
        if method == "POST":
            if in_dev:
                ...
                if not new_password:
                    raise serializers.ValidationError(
                        {
                            "password": "Informe uma senha em ambiente de desenvolvimento."
                        }
                    )
                password_validation.validate_password(
                    new_password, self.instance or (request.user if request else None)
                )
            else:
    ...
    def validate_avatar(self, value):
        ...
        if content_type and content_type not in valid_types:
            raise serializers.ValidationError(
                "Formato inválido. Use JPEG, JPG, PNG ou WEBP."
            )
        return value
