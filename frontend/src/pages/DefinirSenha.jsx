...
export default function DefinirSenha() {
    ...
    try {
      ...
      let readable =
        typeof data === "object"
          ...
          : err.message;
      setVariant("danger");
      setMsg(readable || "Falha ao salvar a senha.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const Requirement = ({ ok, children }) => (
    <div className={`d-flex align-items-start mb-1 ${ok ? "text-success" : "text-muted"}`}>
      <span
        aria-hidden
        className="me-2"
        style={{
          ...
          <Card.Body className="p-4 p-sm-5">
            ...
            {!uid || !token ? (
              <Alert variant="danger" className="mt-3">
                Link inválido ou incompleto. Solicite um novo e-mail de definição de senha.
              </Alert>
            ) : (
              <Form onSubmit={onSubmit} noValidate>
                ...
                <div className="mb-3">
                  ...
                  <Requirement ok={info.checks.upper}>Pelo menos 1 letra maiúscula (A-Z)</Requirement>
                  <Requirement ok={info.checks.lower}>Pelo menos 1 letra minúscula (a-z)</Requirement>
                  <Requirement ok={info.checks.digit}>Pelo menos 1 número (0-9)</Requirement>
                  <Requirement ok={info.checks.symbol}>Pelo menos 1 símbolo (! @ # …)</Requirement>
                </div>
